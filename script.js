document.addEventListener('DOMContentLoaded', () => {
    ...
    if (feedbackForm) {
        feedbackForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}/backend/submit-feedback`, {
                ...
                submitBtn.innerText = 'Submit Feedback'; submitBtn.disabled = false;
            }
        });
    }
    ...
    if (bookingForm) {
        if(!isLoggedIn) {
            ...
            bookingForm.addEventListener('submit', async function(e) {
                ...
                try {
                    const res = await fetch(`${API_BASE_URL}/backend/submit-booking`, {
                        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data)
                    });
                    const result = await res.json();
                    if (res.ok && result.success) {
                        bookingForm.style.display = 'none';
                        if(successMessage) { successMessage.style.display = 'block'; successMessage.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
                    } else throw new Error(result.message);
                } catch (err) {
                    alert(err.message || 'Error processing request.');
                    submitBtn.innerText = 'Submit Booking Request'; submitBtn.style.opacity = '1'; submitBtn.style.cursor = 'pointer'; submitBtn.disabled = false;
                }
            });
        }
    }
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async function(e) {
            ...
            const originalText = submitBtn.innerHTML; submitBtn.innerHTML = 'Sending...'; submitBtn.disabled = true;

            const data = Object.fromEntries(new FormData(contactForm));
            try {
                const res = await fetch(`${API_BASE_URL}/backend/submit-contact`, {
                    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data)
                });
                if (res.ok) {
                ...
                alert('Network Error. Please try again.'); submitBtn.innerHTML = originalText; submitBtn.disabled = false;
            }
        });
    }
    ...
    if (affiliateForm) {
        affiliateForm.addEventListener('submit', async function(e) {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}/backend/submit-affiliate`, {
                    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data)
                });
                if (res.ok) {
                ...
                alert('Network Error. Please try again.'); submitBtn.innerText = 'Apply Now'; submitBtn.disabled = false;
            }
        });
    }
});
